...
def student_dashboard():
    ...
    for i, sub_code in enumerate(subjects):
         sub = sub_code['subjects']
         sid = sub['subject_id']

         stats = stats_map.get(sid, {"total":0, "attended":0} )
         def unenroll_btn(sid, sub_name):
            if st.button(
         ...
         with cols[i % 2]:
             subject_card(
                 ...
                 footer_callback=lambda: unenroll_btn(sid, sub['name'])
...
def student_screen():
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name", placeholder="E.g. Prakhar Garg")
